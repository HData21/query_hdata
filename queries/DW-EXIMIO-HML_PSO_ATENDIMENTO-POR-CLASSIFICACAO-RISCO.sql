-- DW-EXIMIO-HML_PSO_ATENDIMENTO-POR-CLASSIFICACAO-RISCO
...
TAB_ATENDIMENTOS_QTD AS (
    SELECT DISTINCT 
	    DCR.PK_CLASSIFICACAO_RISCO,
    	DCR.NM_CLASSIFICACAO_RISCO_COR,
        COUNT(FA.PK_ATENDIMENTO) OVER (PARTITION BY DCR.PK_CLASSIFICACAO_RISCO) QTD_ATENDIMENTOS_FILTRO,
        COUNT(FA.PK_ATENDIMENTO) OVER () QTD_ATENDIMENTOS_GLOBAL
    FROM TAB_ATENDIMENTOS_FILTRADOS FA
    LEFT JOIN DIM_CLASSIFICACAO_RISCO DCR ON 
    	DCR.PK_CLASSIFICACAO_RISCO = FA.SK_CLASSIFICACAO_RISCO
),
TAB_ATENDIMENTOS_TX AS (
    SELECT
    	*,
    	CASE WHEN QTD_ATENDIMENTOS_GLOBAL = 0 THEN 0 ELSE ROUND( (QTD_ATENDIMENTOS_FILTRO::FLOAT/QTD_ATENDIMENTOS_GLOBAL::FLOAT)::NUMERIC * 100, 2) END TX_ATENDIMENTOS_FILTRO
